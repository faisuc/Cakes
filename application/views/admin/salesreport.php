                ...
                <aside>
                    <div id="sidebar"  class="nav-collapse ">
                        ...
                        <ul class="sidebar-menu" id="nav-accordion">
                            ...
                            <li class="sub-menu">
                                <a class="active" href="<?php echo base_url() . 'SalesReport' ?>">
            ...
            <section id="main-content">
                <section class="wrapper">
                    ...
                    if (isset($_POST['submitDate'])) {
                        ...
                        $totalAmountQuery = $this->db->query("SELECT SUM(`total2`) as `totalAmount` FROM `cart` WHERE `order_date2` LIKE '%$date%' AND `status2` = 'approved'");

                        $totalCustomersQuery = $this->db->query("SELECT COUNT(`user_id2`) as `totalCustomers` FROM `cart` WHERE `order_date2` LIKE '%$date%' AND `status2` = 'approved' GROUP BY `user_id2`");

                        $totalOrdersQuery = $this->db->query("SELECT SUM(`qty2`) as `totalOrders` FROM `cart` WHERE `order_date2` LIKE '%$date%' AND `status2` = 'approved'");
                        $totalProductsQuery = $this->db->query("SELECT SUM(`qty2`) as `totalProducts` FROM `cart` WHERE `order_date2` LIKE '%$date%' AND `status2` = 'approved'");
                        echo "<br><br>";
                        ...
                        <tbody>
                        <td>' . $totalAmountQuery->row()->totalAmount . '</td>
                        <td>' . $totalCustomersQuery->row()->totalCustomers . '</td>
                        <td>' . $totalOrdersQuery->row()->totalOrders . '</td>
                        <td>' . $totalProductsQuery->row()->totalProducts . '</td>
                        </tbody>
                        </table>';
                    }
                    ?>
